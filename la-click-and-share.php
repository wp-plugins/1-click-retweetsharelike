...
function lacands_readOptionsValuesFromWPDatabase() {
    ...
    global $lacands_opt_widget_display_password_protected;
...
function lacands_writeOptionsValuesToWPDatabase($option) {
	...
	if($option == 'default') {
        ...
        add_option('lacands-html-widget-display_password_protected',    0);
		update_option('lacands-html-version-number', 					$lacands_version_number);
	...
	if ($show_widget) {
		...
		$lacands_widget_display_cntrs = '<div style="'.$position.';">
											<iframe
												...
												src="http://linksalpha.com/social?'.$args_data.'">
											</iframe>
										</div>';
		if($show) {
...
function lacands_googlebuzz_langs() {
	$langs = array();
	$response_full = lacands_http_post("http://www.linksalpha.com/a/socialbuttonlangs", array('type'=>'googlebuzz'));
	$response_code = $response_full[0];
	if ($response_code == 200) {
		$response = lacandsnw_networkpub_json_decode($response_full[1]);
		foreach($response as $key=>$val) {
